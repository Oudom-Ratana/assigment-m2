...
export default function ProductGrid() {
  ...
  return (
    <div style={{ backgroundColor: COLORS.secondary }} className="min-h-screen py-10">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 justify-items-center">
          {currentItems.map((item) => (
            <EcommerceProductCard
              key={item.id}
              image_url={item.image_url ?? `https://placehold.co/800x600?text=${encodeURIComponent(item.name)}`}
              name={item.name}
        ...
        <div className="flex items-center justify-center gap-2 mt-10 flex-wrap">
          <button
            ...
            style={{
              ...
              color: page === 1 ? COLORS.accent + "66" : COLORS.primary,
            }}
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button
              ...
              style={
                p === page
                  ? { backgroundColor: COLORS.primary, color: COLORS.secondary, borderColor: COLORS.primary }
                  : { color: COLORS.accent, borderColor: COLORS.primary + "40" }
              }
              className={cn(
          ...
          <button
            ...
            style={{
              borderColor: COLORS.primary,
              color: page === totalPages ? COLORS.accent + "66" : COLORS.primary,
            }}
            className="h-10 w-10 rounded-lg border flex items-center justify-center transition-all disabled:cursor-not-allowed hover:opacity-80"
          >
